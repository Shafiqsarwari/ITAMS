document.addEventListener("DOMContentLoaded", () => {
  ...
  const syncUserOfficeScope = (form) => {
    const role = form.querySelector("[data-user-role-select]");
    const office = form.querySelector("[data-user-office-select]");
    const allOffices = office?.querySelector("[data-admin-all-offices]");
    if (!role || !office || !allOffices) return;

    const canSelectAllOffices = role.value === "admin";
    allOffices.hidden = !canSelectAllOffices;
    allOffices.disabled = !canSelectAllOffices;
    if (!canSelectAllOffices && office.value === allOffices.value) {
      office.value = "";
    }
  };
  document.querySelectorAll("form").forEach((form) => {
    if (!form.querySelector("[data-user-role-select]")) return;
    form
      .querySelector("[data-user-role-select]")
      ?.addEventListener("change", () => syncUserOfficeScope(form));
    syncUserOfficeScope(form);
  });
...
const mfaManagementModal = document.querySelector("[data-mfa-auto-open]");
if (mfaManagementModal) {
  if (mfaManagementModal.dataset.mfaAutoOpen === "true" && window.bootstrap) {
    window.bootstrap.Modal.getOrCreateInstance(mfaManagementModal).show();
  }
}
...
const deleteRequestModalElement = document.querySelector("[data-delete-request-modal]");
const deleteRequestModal =
  deleteRequestModalElement && window.bootstrap
    ? window.bootstrap.Modal.getOrCreateInstance(deleteRequestModalElement)
    : null;

document.addEventListener("click", (event) => {
  const trigger = event.target.closest("[data-delete-request]");
  if (!trigger || !deleteRequestModalElement || !deleteRequestModal) return;

  let ids = [];
  if (trigger.dataset.bulkSource) {
    ids = [
      ...document.querySelectorAll(
        `[data-bulk-select-item][form="${trigger.dataset.bulkSource}"]:checked`,
      ),
    ].map((input) => input.value);
  } else if (trigger.dataset.entityId) {
    ids = [trigger.dataset.entityId];
  }
  ids = [...new Set(ids.filter((id) => /^\d+$/.test(id) && Number(id) > 0))];
  if (!ids.length) return;

  const typeInput = deleteRequestModalElement.querySelector(
    "[data-delete-request-type]",
  );
  const idsContainer = deleteRequestModalElement.querySelector(
    "[data-delete-request-ids]",
  );
  const label = deleteRequestModalElement.querySelector(
    "[data-delete-request-label]",
  );
  if (typeInput) typeInput.value = trigger.dataset.entityType || "";
  if (idsContainer) {
  ...
  if (label) {
    label.textContent =
      ids.length > 1
        ...
        : trigger.dataset.entityLabel || "the selected item";
  }
  deleteRequestModal.show();
});

document.querySelectorAll("[data-mfa-qr]").forEach((qrTarget) => {
  if (typeof window.qrcode !== "function") return;
  const qr = window.qrcode(0, "M");
  qr.addData(qrTarget.dataset.mfaUri || "");
  qr.make();
  qrTarget.innerHTML = qr.createSvgTag({ cellSize: 5, margin: 4, scalable: true });
});

document.querySelectorAll("[data-copy-recovery-codes]").forEach((button) => {
  button.addEventListener("click", async (event) => {
    const container = event.currentTarget.closest("[data-mfa-container]") || document;
    const codes = [...container.querySelectorAll("[data-mfa-recovery-codes] code")]
      .map((element) => element.textContent.trim())
      .join("\n");
    if (!codes) return;
    await navigator.clipboard.writeText(codes);
    event.currentTarget.innerHTML = '<i class="bi bi-check2"></i> Copied';
  });
});
